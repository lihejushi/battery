...
namespace XT.MVC.Core.Encrypt
{
    ...
    public class DesHelper
    {
            ...
            public static string DesKey
            {
                get
                {
                    try
                    {
                        ...
                        if (HostAllows.IndexOf(host) > -1)
                            return Convert.ToBase64String(new byte[] { 136, 59, 38, 91, 29, 28, 21, 217 });
                        throw new Exception(Exmessage);
                    }
                    catch
                    {
                        return Convert.ToBase64String(new byte[] { 21, 59, 38, 91, 29, 28, 21, 21 });
                    }
                }
            }
        ...
        internal class Des
        {
            ...
            {
                this.mKey = string.IsNullOrEmpty(key) ? DefaultSecretKey.DesKey : key;
                this.mIV = string.IsNullOrEmpty(iv) ? DefaultSecretKey.DesKey : iv;
            }
