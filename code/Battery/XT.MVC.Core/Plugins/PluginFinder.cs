...
namespace XT.MVC.Core.Plugins
{
    ...
    public class PluginFinder : IPluginFinder
    {
        ...
        /// <param name="targetId">Store identifier to check</param>
        /// <returns>true - available; false - no</returns>
        public virtual bool AuthenticateApp(PluginDescriptor pluginDescriptor, int targetId)
        {
            ...
            if (pluginDescriptor.LimitedToApps.Count == 0)
                return true;

            return pluginDescriptor.LimitedToApps.Contains(targetId);
        }
        ...
        /// <param name="targetId">Load records allowed only in a specified store; pass 0 to load all records</param>
        /// <returns>Plugins</returns>
        public virtual IEnumerable<T> GetPlugins<T>(bool installedOnly = true, int targetId = 0) where T : class, IPlugin
        {
            ...
            foreach (var plugin in _plugins)
                if (typeof(T).IsAssignableFrom(plugin.PluginType))
                    if (!installedOnly || plugin.Installed)
                        if (AuthenticateApp(plugin, targetId))
        ...
        public virtual IEnumerable<PluginDescriptor> GetPluginDescriptors(bool installedOnly = true, int targetId = 0)
        {
            EnsurePluginsAreLoaded();
        ...
        /// <param name="targetId">Load records allowed only in a specified store; pass 0 to load all records</param>
        /// <returns>Plugin descriptors</returns>
        public virtual IEnumerable<PluginDescriptor> GetPluginDescriptors<T>(bool installedOnly = true, int targetId = 0) 
